...
def build_parser():
    ...
    p.add_argument("--abund-z", type=float, default=2.58, help="Z-score threshold used for quality filtering based on hash abundance (QC)")
    p.add_argument("--min-global-abund", type=float, default=0.05, help="Minimum abundance of a global hash to be included in global distance calculation (QC)")
    p.add_argument("--max-global-dist", type=float, default=0.5, help="Maximum fraction of sample hashes missing from global hashes after abundance filtering (QC)")
...
def main(argv=None):
    ...
    try:
        ...
        # --- Calculate pairwise distances within clusters with new samples ---
        if args.pairwise:
            calculate_cluster_distances(mh_fx.keys(), mh_clust, dist_cache, args.outdir)
        ...
        logger.info(f"floc completed successfully in {elapsed:.1f}s")
        return 0
